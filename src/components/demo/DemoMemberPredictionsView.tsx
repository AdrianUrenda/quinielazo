...
const DemoMemberPredictionsView = ({ memberId, memberName, onBack }: Props) => {
  const { data: predictions, isLoading: loadingPredictions } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("demo_predictions").select("*").eq("user_id", memberId);
  ...
  const matchesWithPredictions = liguillaMatches.filter((m) => predictionMap.has(m.id));
              ...
              <motion.div key={match.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="card-elevated rounded-xl p-4 sm:p-5">
                <div className="mb-3 flex items-center justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    <span className="truncate text-xs font-display tracking-wider text-primary">{getDemoStageLabel(match.round_label)}{match.leg_label ? ` · ${match.leg_label}` : ""}</span>
                  </div>
